...
#include "scarecrow-desktop-thumbnail-script.h"
...
typedef struct {
  ...
  char *infile;
  char *infile_tmp; /* the host version of /tmp/scarecrow-desktop-file-to-thumbnail.* */
  char *outfile;
	   ...
	   ScriptExec  *script)
{
  ...
  /* Add scarecrow-desktop's install prefix if needed */
  ...
  if (exec->sandbox)
    {
      ...
      tmpl = g_strdup ("/tmp/scarecrow-desktop-thumbnailer-XXXXXX");
      ...
      exec->outfile = g_build_filename (exec->outdir, "scarecrow-desktop-thumbnailer.png", NULL);
      basename = g_file_get_basename (file);

      if (basename)
        infile = basename;
      else
        infile = "scarecrow-desktop-file-to-thumbnail";

      exec->infile_tmp = g_build_filename (exec->outdir, infile, NULL);

      exec->s_infile = g_build_filename ("/tmp/", infile, NULL);
      exec->s_outfile = g_build_filename ("/tmp/", "scarecrow-desktop-thumbnailer.png", NULL);
    }
  else
#endif
    {
